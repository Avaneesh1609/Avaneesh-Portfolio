...
// ✅ ADDED: Type one character at a time, then call callback
function typingEffect(el, text, callback) {
    let i = 0;
    el.textContent = "";
    const type = setInterval(() => {
        ...
        if (i === text.length) {
            clearInterval(type);
            setTimeout(callback, 3000); // ✅ Wait 5 seconds before erasing
        }
    }, 80); // typing speed per character
}

// ✅ ADDED: Erase one character at a time, then call callback
function erasingEffect(el, callback) {
...
// ✅ ADDED: Loop through titles forever
function cycle(el) {
    typingEffect(el, titles[currentIndex], () => {
        erasingEffect(el, () => {
